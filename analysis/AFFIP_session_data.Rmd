...
--> siehe 5.: Ja, längeres Arbeiten an spezifischen Zielen reduziert das Ausmaß der Hilfestellung und steigert die Wahrscheinlichkeit für das meistern der Zielebene. Aber längere Ketten sind auch mit einem niedrigeren Expertenlevel verbunden. Dies ist kontrolliert für die signifikante Kovariaten Alter, IQ, Vor/Nach Corona Lockdown
...
--> siehe 4.4.: es gibt 5 Cluster von Zielen mit denen begonnen wird. Mit drei großen Clustern: Blickkontakt + Imitation (n=29), Blickkontakt + Aufmerksamkeit (n=13) und expressive Sprache + Blickkontakt (n = 10). Diese sind unabhängig von Kovariaten
--> siehe 4.3.2.: die Anzahl der Sitzungen bis mit der Arbeit an Therapiezielen begonnen werden beeinflusst durch höhere Alter, höherer IQ, und Beginn der Sitzungen vor dem Corona Lockdown 
...
##January 2026
	...
	1. Welche Faktoren beeinflussen Beginn der Arbeit an Zielen? 
	...
	2. Cluster der Therpapieziele zum Einstieg
		- Eher 3 große Cluster Modellieren
		- -> 5 Clusters are more useful to get a language cluster
		- Einfluss Alter, Geschlecht, Parent education, Migrationsstatus, Corona, IQ, SES - nicht-parametrische Testung
		- --> keinen Einfluss auf Start Cluster
	3. Session Verlauf - anhand der Kettenmodellierung
		a. Ketten auf Ebene der Ziele
...
table(df_session$help) %>%
  kbl(caption = "help: Ausmaß Hilfestellung (0 = Hilfe notwendig, 1 = mit Hilfestellung, 2 = selbstständig)") %>%
  kable_classic(full_width = T, html_font = "Cambria") 
...
# applies fucntions above in loop (partiicpants) of loops (aims per participant)
...
table_chainlength<-table_chainlength[order(table_chainlength[,1],decreasing = T),]
...
lmm<-lmer(scale(chain_help_mean)~scale(chain_length)+scale(full_iq)+gender+scale(agedemo)+
...
lmm<-lmer(scale(chain_achieve_mean)~scale(chain_length)+scale(full_iq)+gender+scale(agedemo)+
            as.factor(chain_after_lockdown_percentage)+(1|id),data=lmm_data)
fun_table_lmm(lmm,caption="achieve outcome: effect of chain length")

lmm<-lmer(scale(chain_mastery_mean)~scale(chain_length)+scale(full_iq)+gender+scale(agedemo)+
            as.factor(chain_after_lockdown_percentage)+(1|id),data=lmm_data)
